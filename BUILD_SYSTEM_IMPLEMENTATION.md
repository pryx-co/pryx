...
- `make build-ui` - Build React/TypeScript UI
- `make build-edge` - Build Cloudflare Workers

- `make test` - Run all tests
- `make test-host` - Run Rust tests
- `make test-runtime` - Run Go tests
- `make test-ui` - Run React UI tests
- `make test-edge` - Run Cloudflare Workers tests

- `make lint` - Run all linters
- `make lint-host` - Run Rust linters (rustfmt, clippy)
- `make lint-runtime` - Run Go linters (gofmt, golangci-lint)
- `make lint-ui` - Run React/TypeScript linters (eslint, prettier)
- `make lint-edge` - Run Cloudflare Workers linters
...
4. **Test UI (React)** - Runs eslint, prettier, tests, and uploads coverage
5. **Test edge (Workers)** - Runs eslint, prettier, tests
6. **Build all** - Builds all components for Linux, macOS, Windows
7. **Security audit** - Runs cargo audit, go dependency audit, npm audit
...
- prettier - Format JavaScript, TypeScript, JSX, TSX, JSON, CSS, YAML, Markdown
...
1. **Create component directories** (`host/`, `runtime/`, `ui/`, `edge/`)
