        ...
        if req.method == "permission.request" {
            let ans = {
                ...
                if let Some(app) = app_guard.as_ref() {
                    // Parse params
                    let description = req.params.get("description").and_then(|v| v.as_str()).unwrap_or("Unknown Action");
                    let _intent = req.params.get("intent").and_then(|v| v.as_str()).unwrap_or("Requested by Runtime");
            ...
            self.send_response(resp).await?;
             ...
             let notification_result = {
                 ...
                 let app_guard = self.app_handle.lock().unwrap();
                 if let Some(app) = app_guard.as_ref() {
                     let title = req.params.get("title").and_then(|v| v.as_str()).unwrap_or("Pryx Notification");
             ...
             let text = req.params.get("text").and_then(|v| v.as_str()).unwrap_or("").to_string();
             let success = {
                let app_guard = self.app_handle.lock().unwrap();
                if let Some(app) = app_guard.as_ref() {
