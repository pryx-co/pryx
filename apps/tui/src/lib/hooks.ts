...
import { Effect, Fiber, Runtime, Stream, Context, Layer, ManagedRuntime } from "effect";
 ...
 */
export function useEffectStream<A, E = never>(
        ...
        const fiber = AppRuntime.runFork(
            stream.pipe(
                Stream.runForEach((item) =>
                    Effect.sync(() => setItems((prev) => [...prev, item]))
                )
            )
        );
 ...
 */
export function useEffectService<I, S>(
    tag: Context.Tag<I, S>
): Accessor<S | undefined> {
    ...
    try {
        ...
        onMount(() => {
            ...
            AppRuntime.runPromise(tag as any).then((svc) => {
